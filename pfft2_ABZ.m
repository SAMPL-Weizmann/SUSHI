function [ Xf ] = pfft2_ABZ( X, M, iFlag, varargin )
...
switch lower(iFlag)
    ...
    case 'fft_h'
        ...
        % ----------------------------------------------------- %   
        % Stage 1: Z = F'*X
        X1 = M*ifft(X, M);               % Factor M is needed for x = pfft{fft{x}} (M is N)
%         X = ifft(X, M);               % Omit factor of M for pFISTA to work well???
        
        % Stage 2: Xf = Z*conj(F)
        Xf = ctranspose(fft(X1', M));    % Conjugate transpose on the fft
        
        % new version
        padsize = (M-Morig)/2;
        Xpad = fftshift(padarray(fftshift(X),[padsize,padsize]));
        Xf = fftshift(ifft2(Xpad)*M^2);%  do we need fftshift ???
    case 'fft_h_n_mid'
